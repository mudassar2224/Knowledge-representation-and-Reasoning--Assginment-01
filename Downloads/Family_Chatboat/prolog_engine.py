# Pytholog wrapper for the family knowledge base.
...
def load_kb():
    """Load family_kb.pl into one module-level Pytholog knowledge base."""
    global _kb
...
def query_yes_no(relation, args):
    ...
    if len(args) == 2 and all(is_safe_atom(normalize_atom(arg)) for arg in args):
        ...
        right_matches = _values(query(relation, [left, "X"]))
        if right in right_matches:
            return True

        left_matches = _values(query(relation, ["X", right]))
        if left in left_matches:
            return True
